...
*   **Case Var. (ケース変数)**
    `caseNum`, `caseTitle`, `SLA`, `severity`, `createdAt` など、ケース本体に保存されている代表的なメタデータを一覧表示します。
    - 値が空の変数は **デフォルトで非表示** になります。「🔽 空の変数をすべて表示」ボタンで展開でき、「🔼 空の変数を隠す」で折り畳めます。
    - すべての変数が空の場合は、先頭2件のみ表示されます。
*   **Sys Temp. (システムテンプレート)**
    登録済みのすべての System Template を変数として一覧表示します。テンプレートタイトルと ID（例: `attentions`, `teamsDisclaimer`）が表示され、`{{attentions}}` のような形でコピーしてテンプレート内から参照できます。
...
## 🛠️ カスタム定型文 (Custom Template Phrases)

各テンプレートの初期テキスト（LLM プロンプトやメール本文など）は、個別にカスタマイズして保存可能です。

*   **保存先**: `localStorage("dfm_ninja_custom_phrase_<テンプレートID>")`
*   **優先順位**: 
    1.  `localStorage` 内のカスタム定型文
    2.  テンプレート（HTML）内の `<noscript>` 定型文
*   **編集方法**: Settings モーダルのテンプレート管理画面、またはテンプレート詳細画面から編集可能です。
*   **動的リセット**: 定型文に紐づくフィールドの 🔄（リセット）ボタンは、現在読み込まれている定型文ソースに戻します。カスタム定型文が保存されている場合はその内容が優先され、Step の保存状態とも同期されます。
...
| `SLA` | SLAステータス（時間形式の場合は残り時間、"Expired"の場合はそのまま、未定義時は"Met"） | Met / 1d 2h 3m / Expired |
...
※ ケース本体のメタデータは、同モーダルの **Case Var.** から確認・コピーできます。
...
| `slaOrg` | `string` | 元のSLAの生の値（例: `1d 2h 3m 4s`, `Expired`, 空文字） |
| `SLA` | `string` | フォーマット済みのSLA（`MM/DD HH:MM` 形式、`Expired`、または `Met`） |
