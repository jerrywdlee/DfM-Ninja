...
*   **Stage Var. (ステージ内部変数)**
    現在開かれている Stage に含まれる全 Step の HTML をスキャンし、`name="..."` 属性で定義された変数を一覧化します。現在の入力値プレビュー（先頭20文字）も同時に表示され、未解決の変数タグ `{{...}}` 等は自動で空白として扱われます。

また、リストの右側にある「🧲」マグネットボタンをクリックすることで、`{{変数名}}` の形式で簡単にクリップボードへコピー（コピー完了通知付き）が可能です。
